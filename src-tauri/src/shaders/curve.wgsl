...
//   x_affine = X / Z² ,  y_affine = Y / Z³
//   Cost: 1 fe_inv (~270 mul) + 3 fe_mul + 1 fe_square.
//   This is the per-step bottleneck — batching (Phase 5) is needed to amortize.
// -----------------------------------------------------------------------------
fn jac_to_affine(p: JacobianPoint) -> AffinePoint {
    let z_inv = fe_inv(p.z);                  // Z⁻¹
    let z2_inv = fe_square(z_inv);             // Z⁻²
    let z3_inv = fe_mul(z2_inv, z_inv);        // Z⁻³
    var r: AffinePoint;
    r.x = fe_mul(p.x, z2_inv);                 // X / Z²
    r.y = fe_mul(p.y, z3_inv);                 // Y / Z³
    return r;
}
