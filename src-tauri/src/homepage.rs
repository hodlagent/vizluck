...
/// The unavoidable core: turn a private key into its compressed pubkey's
/// hash160.  Scalar multiplication is required to compute a hash160, so this is
/// the one cost every check must pay.  Returns `(hash160, public_key)` — the
/// `PublicKey` lets the full path reuse the already-computed point.
fn compute_hash160(key: &[u8; 32]) -> Result<([u8; 20], secp256k1::PublicKey), String> {
    use secp256k1::{PublicKey, Secp256k1, SecretKey};
    let secp = Secp256k1::new();
    let sk = SecretKey::from_byte_array(*key).map_err(|e| format!("invalid scalar: {e}"))?;
    let pk = PublicKey::from_secret_key(&secp, &sk);
    let h160 = crate::btc::hash160(&pk.serialize());
    Ok((h160, pk))
}
...
fn derive_key_info(
    ...
    let legacy_address = crate::btc::p2pkh(&compressed);
    ...
    let address_match = expected_hash160.map(|target| pubkey_hash160 == target);
...
/// Lightweight auto-mode tick: sample a random key in the range and compute only
/// its hash160 (plus the puzzle comparison).  Skips address encoding, BIP32, and
/// the uncompressed pubkey — everything the grid doesn't need.  On a match the
/// result is saved and the path returned so the frontend can pause + celebrate.
#[tauri::command]
pub fn random_and_hash160(
    ...
    loop {
        ...
        let (h160, _pk) = match compute_hash160(&key) {
            Ok(r) => r,
            Err(_) => {
                attempts += 1;
                if attempts >= 1000 {
        ...
        let address_match = expected_hash160.map(|target| h160 == target);
...
mod tests {
    ...
    fn auto_tick_produces_hash160_in_range() {
        ...
        for _ in 0..50 {
            ...
            assert_eq!(res.pubkey_hash160.len(), 40);
    ...
    fn derive_full_matches_random_and_derive() {
        ...
        assert_eq!(full.pubkey_hash160.len(), 40);
        assert!(!full.legacy_address.is_empty());
        assert!(full.legacy_address.starts_with('1'));
