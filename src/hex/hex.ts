/**
 * Hex tab logic — the original vanilla-TS homepage, moved verbatim out of
 * `main.ts` so `Hex.svelte` can own it.
 *
 * Only the boundaries changed, both marked below:
 *   1. DOM element refs are resolved in `initHex()` instead of at module scope.
 *   2. The `DOMContentLoaded` bootstrap became the exported `initHex()`.
 * Everything in between (state, rendering, hover cycles, the auto loop) is
 * unchanged and deliberately still imperative — see the "two-step" scope.
 */
...
// Resolved in `initHex()`, not here: this module is imported by `Hex.svelte`,
// which happens *before* the component's markup exists — a top-level
// `getElementById` would throw `missing #id`.
let groupSelect: HTMLSelectElement;
let rangeInput: HTMLInputElement;
let btnRandom: HTMLButtonElement;
let btnAuto: HTMLButtonElement;
let gridEl: HTMLElement;
let dividerEl: HTMLElement;
let blocksEl: HTMLElement;
let infoEl: HTMLElement;
let errorEl: HTMLElement;
let toastEl: HTMLElement;
let keysRateEl: HTMLElement;
...
// ── Keys/s subheader ─────────────────────────────────────────────────────────
// Tally every derived key over a 10s rolling window and show the rate next to
// the vizluck title.  An idle gap > IDLE_RESET_MS drops the stale window so a
// fresh scan isn't diluted by dead time.
const RATE_WINDOW_MS = 10_000;
const RATE_IDLE_RESET_MS = 30_000;
let scannedKeys = 0;
let rateWindowStart = performance.now();

/** Count `n` derived keys and refresh the keys/s display when a window closes. */
function recordKeys(n: number) {
  const now = performance.now();
  if (now - rateWindowStart > RATE_IDLE_RESET_MS) {
    scannedKeys = 0;
    rateWindowStart = now;
  }
  scannedKeys += n;
  if (now - rateWindowStart >= RATE_WINDOW_MS) {
    const rate = Math.round(scannedKeys / ((now - rateWindowStart) / 1000));
    keysRateEl.textContent = `${rate} keys/s`;
    scannedKeys = 0;
    rateWindowStart = now;
  }
}
...
// The derived-key cards (private key, xprv, compressed pubkey, compressed BTC
// address), shown persistently at the bottom of the page — one panel per puzzle
// in the active group, or a single panel for a custom-range result.  Every card
// is click-to-copy and every derivation tick refreshes its panel in place.
 ...
 * puzzle's target hash160 (when known) is resolved here so the address card can
 * show its own compressed match status.
 */
...
/**
 * Wire the Hex tab up.  Called from `Hex.svelte`'s `onMount`, which runs after
 * the component's markup is in the DOM — that ordering is exactly why the
 * element refs above are resolved here rather than at module scope.
 *
 * The `wired` guard makes a repeat call (hot reload) harmless: element refs are
 * re-resolved against the current DOM, but listeners and timers bind only once.
 */
let wired = false;

export async function initHex() {
  groupSelect = $<HTMLSelectElement>("group-select");
  rangeInput = $<HTMLInputElement>("range-input");
  btnRandom = $<HTMLButtonElement>("btn-random");
  btnAuto = $<HTMLButtonElement>("btn-auto");
  gridEl = $("grid");
  dividerEl = $("divider");
  blocksEl = $("puzzle-blocks");
  infoEl = $("info");
  errorEl = $("error");
  toastEl = $("toast");
  keysRateEl = $("keys-rate");
